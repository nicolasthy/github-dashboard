...
export async function issueToken(
  ...
  db.prepare("INSERT INTO api_token (token_id, hash, label, created_at) VALUES (?, ?, ?, ?)").run(
    token_id,
    hash,
    label,
    now,
...
export async function verifyToken(db: Database, plaintext: string): Promise<ApiToken | null> {
  ...
  for (const row of rows) {
    ...
    if (matches) {
      // Update last_used_at
      db.prepare("UPDATE api_token SET last_used_at = ? WHERE token_id = ?").run(
